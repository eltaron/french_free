...
function getLatestlang($select, $table, $where , $order, $limit = 5) {
	global $con;
...
function checkItemLang ($select, $from, $value, $and) {
	global $con; 
	$stmt = $con->prepare("SELECT $select FROM $from WHERE $select = ? $and");
	$stmt->execute(array($value));
	$count = $stmt->rowCount();
	return $count;
}
function countItemsLang ($item, $table, $where) {
	global $con;
